  ...
  while (listaAbierta.length != 0) {
    ...
    for (let i = 0; i < listaAbierta.length; i++) {
      if (listaAbierta[i][1] < mejorF) {
        ...
        g = listaAbierta[i][2];
        indiceMejor = i;
        caminoActual = listaAbierta[i][4]; // recupera el camino que llevó hasta ahí
      }
    }
...
function BFS(matriz, x, y) {
  ...
  while (cola.length > 0) {
    ...
    // Control de niveles explorados métricas
    ...
    // Límite para evitar desbordamiento de memoria
